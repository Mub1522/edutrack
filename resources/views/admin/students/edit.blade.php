<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit student') }}: <span class="text-red-700">({{ $student->uuid }})</span> {{ $student->name }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div>
                            ...
                            <br>
                            <form class="max-w-xl mx-auto" action="{{ route('admin.students.update', $student->id) }}"
                                ...
                                <div class="relative z-0 w-full mb-5 group">
                                    <x-input-animated type="text" name="name" id="name" required
                                        label="{{ __('Name') }}"
                                        value="{{ old('name', $student->name) }}"></x-input-animated>
                                </div>
                                <div class="grid md:grid-cols-2 md:gap-6">
                                    <div class="relative z-0 w-full mb-5 group">
                                        <x-input-animated type="email" name="email" id="email" required
                                            label="{{ __('Email') }}"
                                            value="{{ old('email', $student->email) }}"></x-input-animated>
                                    </div>
